...
class Client(object):
    ...
    def _results(
        ...
        if not feed.entries:
            print("Got empty first page; stopping generation")
            return
        ...
        while feed.entries:
            for entry in feed.entries:
                try:
                    ...
                    print("Skipping partial result: %s", e)
            ...
            try:
                ...
                print("Unrecoverable, offsetting more")
                offset += 100
                total_results +=100
                if offset >= total_results:
                    break
    ...
    def _parse_feed(
        ...
        try:
            ...
            if _try_index < self.num_retries:
                time.sleep(2)
                print(f"Warning, error encountered {err}. Sleep+ing.")
